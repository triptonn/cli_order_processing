"""Module holding all authentication related functions"""
...
class Authenticator:
    ...
    def custom_hash(self, value: str):
        ...
        _hashed_value = scrypt.hash(
            password=value,
            salt=self._salt + str.encode(self._username),
            N=self._N,
            r=self._r,
            p=self._p,
        )
    ...
    def set_username(self):
        ...
        while not _username_set:
            ...
            while not _username_equal:
                _username_control = getpass.getpass(
                    "        Benutzernamen erneut eingeben: "
                )
                _username_set = True
    ...
    def set_password(self, user_id: int = 0):
        ...
        if user_id == -1:
            _password = getpass.getpass(
                "        Erster Login :-) Wilkommen! Bitte vergib das 'admin' Passwort: "
            )
        elif user_id > 0:
            _password = getpass.getpass(
                f"        Das neue Passwort für den Benutzer {user_id} lautet: "
            )
        else:
            _password = getpass.getpass(
                "        Wähle ein Passwort für den neuen Benutzer: "
            )
        ...
        while not _password_set:
            ...
            while not _password_equal:
                _password_control = getpass.getpass(
...
class AuthenticatedUser:
    ...
    def __init__(
        ...
        self._super_hash = scrypt.hash(
            self._random_key + password_hash,
            self._salt,
            N=self._N,
            p=self._p,
            r=self._r,
